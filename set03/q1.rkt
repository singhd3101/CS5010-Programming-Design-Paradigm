...
;; SIMULATION FOR SQUASH PRACTICE (SquashPractice1) 
...
(define (initial-world t)
  (make-world
   ...
   (make-racket INIT-X-COORD INIT-Y-COORD 0 0)
   true))
...
;; REPRESENTATION:
;; A World is represented as a (make-world wball wracket paused?)
;; INTERPRETATION:
...
;; IMPLEMENTATION:
(define-struct world (wball wracket paused?))

;; CONSTRCTOR TEMPLATE:
;; (make-world Ball Racket Boolean)
...
;; REPRESENTATION:
;; A Racket is represented as (make-racket x-posr y-posr x-velr y-velr)
;; INTERPRETATION:
...
(define-struct racket (xposr yposr xvelr yvelr))
...
(define racket-test1 (make-racket INIT-X-COORD INIT-Y-COORD 0 0))
(define racket-test2 (make-racket INIT-X-COORD INIT-Y-COORD 2 -6))
...
;; (make-world (make-ball 330 384 0 0)
;;    (make-racket 330 384 0 0) false) = false
;; (make-world (make-ball 330 384 7 0)
;;    (make-racket 330 384 0 0) false) = false
;; (make-world (make-ball 330 384 0 0)
;;    (make-racket 330 289 0 0) false) = false
...
;; EXAMPLES :
;; (racket-state (make-racket 330 384 0 0)) = true
;; (racket-state (make-racket 330 384 7 0)) = false
;; (racket-state (make-racket 330 269 0 0)) = false
...
(begin-for-test
  (check-equal? (racket-state (make-racket 330 384 0 0)) true)
  (check-equal? (racket-state (make-racket 330 384 7 0)) false)
  (check-equal? (racket-state (make-racket 330 269 0 0)) false))
...
;; DESIGN STRATEGY : Combine simpler functions
;; EXAMPLES :
;; (racket-x (make-racket 5 15 4 2)) = 5
...
(begin-for-test
  (check-equal? (racket-x (make-racket 5 15 4 2)) 5))
...
;; DESIGN STRATEGY : Combine simpler functions
;; EXAMPLES :
;; (racket-y (make-racket 5 15 4 2)) = 15
...
(begin-for-test
  (check-equal? (racket-y (make-racket 5 15 4 2)) 15))
...
;; DESIGN STRATEGY : Combine simpler functions
;; EXAMPLES :
;; (racket-vx (make-racket 5 15 4 2)) = 4
...
(begin-for-test
  (check-equal? (racket-vx (make-racket 5 15 4 2)) 4))
...
;; DESIGN STRATEGY : Combine simpler functions
;; EXAMPLES :
;; (racket-vy (make-racket 5 15 4 2)) = 2
...
(begin-for-test
  (check-equal? (racket-vy (make-racket 5 15 4 2)) 2))